...
export const Route = createFileRoute("/admin/structure")({
  head: () => ({ meta: [{ title: "Structure — Admin" }] }),
  component: AdminStructure,
});

// Connected external accounts / integrations the whole system depends on.
const INTEGRATIONS = [
  { name: "Broker (IB)", detail: "Commission + deposit postback", icon: Wallet, connected: false, note: "API pending" },
  { name: "Telegram Bot", detail: "Signal relay + access gate", icon: Bot, connected: true },
  { name: "Supabase", detail: "Central database (leads, members, tenants)", icon: Database, connected: true },
  { name: "Resend", detail: "Email delivery (transactional + newsletter)", icon: Mail, connected: false, note: "Domain + key pending" },
] as const;
  ...
  return (
    <div className="space-y-8">
      <AdminPageHeader
        ...
        sub="Tree: master → partner brands → members. Everything runs centrally through your tree."
      />

      <CreatePartnerForm />

      <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
        <AdminKpiCard label="Partner brands" value={totals.partners} icon={Building2} />
        <AdminKpiCard label="Total members" value={totals.members} icon={Users} tone="ok" />
        <AdminKpiCard label="Deposits" value={formatMoney(totals.deposits, "€")} icon={Wallet} tone="primary" />
        <AdminKpiCard label="Total leads" value={totals.leads} icon={Send} />
      </div>
      ...
      <div className="rounded-2xl border border-white/5 bg-[oklch(0.15_0.045_255)] p-5 lg:p-7">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <div className="text-[11px] text-muted-foreground">Master account · all accounts run under this tree</div>
          </div>
        ...
        <div className="mt-2 space-y-2 border-l border-white/10 pl-5 lg:ml-6">
          {loading && (
            <div className="flex items-center gap-2 py-6 text-sm text-muted-foreground"><Loader2 className="h-4 w-4 animate-spin" /> Loading structure…</div>
          )}
          {!loading && partners.length === 0 && (
            <div className="rounded-xl border border-dashed border-white/10 px-4 py-8 text-center text-sm text-muted-foreground">
              No partner brands yet. Create the first one in the <span className="font-semibold text-foreground/80">White label</span> section.
            </div>
          )}
          {partners.map((p) => {
                ...
                <div className="rounded-xl border border-white/5 bg-white/[0.03]">
                  ...
                  >
                    ...
                    <div className="hidden items-center gap-4 pr-2 text-right sm:flex">
                      ...
                      <div><div className="text-sm font-bold tabular-nums">{formatMoney(p.totalDeposits, "€")}</div><div className="text-[10px] text-muted-foreground">Deposits</div></div>
                  ...
                  {isOpen && (
                    <div className="border-t border-white/5 px-4 py-3">
                      <div className="flex flex-wrap items-center gap-2 text-[11px]">
                        ...
                        <span className="inline-flex items-center gap-1 rounded-md bg-white/5 px-2 py-1"><Send className="h-3 w-3" /> {p.clicks} clicks · {p.leads} leads</span>
      ...
      <div className="rounded-2xl border border-white/5 bg-[oklch(0.15_0.045_255)] p-5">
        <div className="mb-4 text-[11px] font-semibold uppercase tracking-[0.18em] text-muted-foreground">Connected accounts & integrations</div>
        <div className="grid gap-2 sm:grid-cols-2">
          {INTEGRATIONS.map((it) => (
            <div key={it.name} className="flex items-center gap-3 rounded-xl bg-white/[0.03] p-3">
              ...
              {it.connected ? (
                <span className="inline-flex items-center gap-1 rounded-md bg-[oklch(0.78_0.16_150)]/15 px-2 py-1 text-[11px] font-semibold text-[oklch(0.8_0.16_150)]"><CheckCircle2 className="h-3 w-3" /> connected</span>
              ) : (
                <span className="inline-flex items-center gap-1 rounded-md bg-amber-500/15 px-2 py-1 text-[11px] font-semibold text-amber-400" title={it.note}><XCircle className="h-3 w-3" /> {it.note ?? "pending"}</span>
              )}
            </div>
          ))}
        </div>
