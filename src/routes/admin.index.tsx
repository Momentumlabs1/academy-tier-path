...
const QUICK = [
  { to: "/admin/structure", label: "Struktur", sub: "Stammbaum & Accounts", icon: Network },
  { to: "/admin/tenants", label: "White-Label", sub: "Marken verwalten", icon: Palette },
  { to: "/admin/signals", label: "Signal Relay", sub: "Bot & Kanäle", icon: Radio },
  { to: "/admin/members", label: "Members", sub: "Nutzer verwalten", icon: Users },
] as const;
...
function AdminOverview() {
  ...
  return (
    <div className="space-y-8">
      <AdminPageHeader
        title="Overview"
        sub={`Stand ${new Date().toLocaleDateString("de-DE", { day: "2-digit", month: "short", year: "numeric" })}`}
        action={
          <Link to="/admin/structure" className="inline-flex items-center gap-1.5 rounded-full bg-primary px-4 py-2 text-sm font-semibold text-primary-foreground transition-opacity hover:opacity-90">
            Struktur ansehen <ArrowRight className="h-3.5 w-3.5" />
          </Link>
        }
      />
      ...
      <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
        <AdminKpiCard label="Kunden gesamt" value={loading ? "…" : totals.members} icon={Users} />
        <AdminKpiCard label="Leads gesamt" value={loading ? "…" : totals.leads} icon={Send} tone="ok" />
        <AdminKpiCard label="Einzahlungen" value={loading ? "…" : formatMoney(totals.deposits, "€")} icon={Wallet} tone="primary" />
        <AdminKpiCard label="Partner-Marken" value={loading ? "…" : totals.partners} icon={Palette} />
      </div>
      ...
      <div className="grid gap-4 lg:grid-cols-[1fr_360px]">
        ...
        <div className="rounded-2xl border border-white/5 bg-[oklch(0.15_0.045_255)] p-5">
          <div className="mb-4 text-[11px] font-semibold uppercase tracking-[0.18em] text-muted-foreground">Aktivität</div>
          <div className="flex flex-col items-center justify-center gap-2 py-12 text-center">
            <Inbox className="h-8 w-8 text-muted-foreground/40" />
            <div className="text-sm text-muted-foreground">Noch keine Aktivität.</div>
            <div className="text-xs text-muted-foreground/70">Registrierungen, Einzahlungen und Tier-Wechsel erscheinen hier live.</div>
          </div>
        </div>
        ...
        <div className="rounded-2xl border border-white/5 bg-[oklch(0.15_0.045_255)] p-5">
          <div className="mb-4 flex items-center justify-between">
            <div className="text-[11px] font-semibold uppercase tracking-[0.18em] text-muted-foreground">Partner-Marken</div>
            <Link to="/admin/tenants" className="text-xs font-semibold text-primary hover:underline">Verwalten</Link>
          </div>
          <div className="flex flex-col gap-2">
            {TENANTS.map((t) => (
              <div key={t.slug} className="flex items-center gap-3 rounded-xl bg-white/[0.03] p-2.5">
                ...
                <Link to="/t/$slug" params={{ slug: t.slug }} target="_blank" className="shrink-0 rounded-lg p-1.5 text-muted-foreground hover:bg-white/5 hover:text-foreground" title="Landingpage öffnen">
