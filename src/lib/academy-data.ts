...
export interface Lesson {
  ...
  completed?: boolean;
  /** Self-hosted lesson video URL (Supabase Storage). Undefined = not yet published. */
  videoUrl?: string;
  /** Concrete, per-lesson learning outcomes shown on the detail page. */
  objectives: string[];
}

// Self-hosted lesson videos — Supabase Storage public bucket "lesson-videos".
// Override base via VITE_VIDEO_BASE in .env; defaults to /videos (served from public/).
export const VIDEO_BASE = (((import.meta.env as any).VITE_VIDEO_BASE as string) || "/videos").replace(/\/+$/, "");
const V = (file: string) => VIDEO_BASE + "/" + file;

// Curriculum based on the head trader's course script (Level-2 / orderflow strategy).
export const LESSONS: Lesson[] = [
  { id: "l1", title: "What Is Trading?", description: "Long & short, the markets, and why trading is not investing.", durationMin: 14, tier: "foundation", category: "Foundations", completed: true, videoUrl: V("lesson1.mp4"),
  ...
  { id: "l3", title: "Why 90% of Traders Lose", description: "The avoidable mistakes: no plan, overtrading, revenge trading.", durationMin: 16, tier: "foundation", category: "Risk", completed: false, videoUrl: V("lesson3.mp4"),
  ...
  { id: "l8", title: "What Is Retail Money?", description: "How the crowd trades — and why that's the wrong approach.", durationMin: 13, tier: "operator", category: "Orderflow", videoUrl: V("lesson4.mp4"),
  ...
  { id: "l9", title: "Level 1 vs. Level 2 Data", description: "The information gap between retail and institutions.", durationMin: 15, tier: "operator", category: "Orderflow", videoUrl: V("lesson5.mp4"),
  ...
  { id: "l11", title: "Volume Profile: Value Area, HVN & LVN", description: "Where the market feels comfortable — and where the fast moves happen.", durationMin: 20, tier: "elite", category: "Orderflow", videoUrl: V("video6.mp4"),
